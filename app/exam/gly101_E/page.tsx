...
import { 
  ...
  Award, AlertCircle, X, Calculator, Share2, Search, Info, Home, RefreshCw, User, Lightbulb, Mountain
} from 'lucide-react';
...
const PulsarModal = ({ title, message, onConfirm, onCancel, confirmText="Confirm", cancelText="Cancel", isDestructive=false, singleButton=false }: any) => (
  <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
     <div className="bg-[#111113] border border-zinc-800 p-6 rounded-2xl w-full max-w-sm text-center shadow-2xl scale-100">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4 ${isDestructive ? 'bg-red-500/10' : 'bg-amber-500/10'}`}>
            {isDestructive ? <AlertCircle className="w-6 h-6 text-red-500"/> : <CheckCircle className="w-6 h-6 text-amber-500"/>}
        </div>
        ...
        <div className="flex gap-3">
           ...
           <button onClick={onConfirm} className={`flex-1 py-3 rounded-xl font-bold text-sm text-white ${isDestructive ? 'bg-red-600 hover:bg-red-500' : 'bg-amber-600 hover:bg-amber-500'}`}>{confirmText}</button>
        </div>
     </div>
  </div>
);
    ...
    <div className="fixed bottom-20 right-4 z-[60] bg-zinc-900 border border-zinc-700 p-4 rounded-2xl shadow-2xl w-64 animate-fade-in-up">
      ...
      <div className="grid grid-cols-4 gap-2">
        {['7','8','9','/','4','5','6','*','1','2','3','-','C','0','=','+'].map(btn => (
          <button key={btn} onClick={() => handlePress(btn)} className={`p-2 rounded-lg font-bold text-sm ${btn === '=' ? 'bg-amber-600 text-white col-span-2' : 'bg-zinc-800 text-zinc-300'}`}>{btn}</button>
        ))}
      </div>
    </div>
  );
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.share) {
            ...
            try { await navigator.share({ title: 'Pulsar CBT Result', text: `I scored ${score}/${questions.length} in GLY 101!`, files: [file] }); } catch (err) { console.log(err); }
        } else {
  ...
  if (!examStarted) return (
    <div className="min-h-screen bg-[#09090b] flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-[#111113] border border-zinc-800 p-8 rounded-3xl text-center shadow-2xl">
        <div className="w-20 h-20 bg-amber-600/20 rounded-full flex items-center justify-center mx-auto mb-6"><Mountain className="w-10 h-10 text-amber-500"/></div>
        <h1 className="text-2xl font-bold text-white mb-2">GLY 101</h1>
        <p className="text-zinc-500 text-sm mb-6">Introduction to Geology</p>
      ...
      <div className="min-h-screen bg-[#09090b] text-white p-6 flex items-center justify-center">
        <div className="w-full max-w-md">
            <div ref={resultCardRef} className="bg-[#111113] border border-zinc-800 p-8 rounded-3xl text-center shadow-2xl mb-6 relative overflow-hidden">
                ...
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-600 mb-8">GLY 101 Score</p>
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <button onClick={handleShare} className="py-4 bg-amber-600 text-white font-bold rounded-xl hover:bg-amber-500 flex items-center justify-center gap-2 transition-colors shadow-lg shadow-amber-900/20"><Share2 className="w-4 h-4"/> Share Result</button>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-zinc-200 font-sans p-4 md:p-6 pb-24 relative">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {isReviewing ? (<div className="px-4 py-1 bg-amber-900/30 border border-amber-500/30 rounded-full text-amber-300 text-xs font-bold uppercase tracking-widest">Review Mode</div>) : (<div className={`font-mono font-bold text-lg ${timeLeft < 300 ? 'text-red-500 animate-pulse' : 'text-amber-400'}`}>{Math.floor(timeLeft/60)}:{String(timeLeft%60).padStart(2,'0')}</div>)}
        <button onClick={() => setShowCalculator(!showCalculator)} className={`p-2 rounded-full ${showCalculator ? 'bg-amber-600 text-white' : 'bg-zinc-800/50 text-zinc-400'}`}><Calculator className="w-5 h-5"/></button>
      </div>
            ...
            {currentQ.display_options.map((opt: any, idx: number) => {
              ...
              if (isReviewing) {
                 if (isCorrectOption) {
                  ...
                  if (isSelected) btnClass = "bg-amber-600 border-amber-600 text-white shadow-lg shadow-amber-900/20"; 
              }
        ...
        <div className="bg-[#111113] border border-zinc-800 p-6 rounded-3xl h-fit">
          ...
          <div className="grid grid-cols-5 gap-2">
            {questions.slice(gridStart, gridEnd).map((q, i) => {
              ...
              if (actualIndex === currentIndex) colorClass = "bg-white text-black border-white ring-2 ring-amber-500";
              else if (isReviewing) {
              ...
              else if (answered) colorClass = "bg-amber-600/20 text-amber-400 border-amber-500/50";
              return (
