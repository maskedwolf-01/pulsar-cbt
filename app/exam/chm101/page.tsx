...
import { 
  ...
  Award, AlertCircle, X, Calculator, Share2, Search, Info, Home, RefreshCw, User, FlaskConical
} from 'lucide-react';
...
export default function ExamPage() {
  ...
  const fetchAndShuffleQuestions = async () => {
    // FETCHING CHM 101 SPECIFICALLY
    const { data, error } = await supabase.from('questions').select('*').eq('course_code', 'CHM 101');
        ...
        <div className="bg-[#111113] border border-zinc-800 p-6 md:p-10 rounded-3xl shadow-lg relative min-h-[500px] flex flex-col">
          ...
          <div className="space-y-3 mb-8">
            {currentQ.display_options.map((opt: any, idx: number) => {
              ...
              } else { if (isSelected) btnClass = "bg-teal-600 border-teal-600 text-white shadow-lg shadow-teal-900/20"; }
              return (
