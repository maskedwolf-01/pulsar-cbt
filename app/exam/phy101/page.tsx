...
import { 
  ...
  Award, AlertCircle, X, Calculator, Share2, Search, Info, Home, RefreshCw, User, Atom
} from 'lucide-react';
...
export default function ExamPage() {
  ...
  const fetchAndShuffleQuestions = async () => {
    // FETCHING PHY 101 SPECIFICALLY
    const { data, error } = await supabase.from('questions').select('*').eq('course_code', 'PHY 101');
        ...
        <div className="bg-[#111113] border border-zinc-800 p-6 md:p-10 rounded-3xl shadow-lg relative min-h-[500px] flex flex-col">
          ...
          <div className="space-y-3 mb-8">
            {currentQ.display_options.map((opt: any, idx: number) => {
              ...
              } else { if (isSelected) btnClass = "bg-yellow-600 border-yellow-600 text-white shadow-lg shadow-yellow-900/20"; }
              return (
