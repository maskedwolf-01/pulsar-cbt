...
import { 
  ...
  Award, AlertCircle, X, Calculator, Share2, Search, Info, Home, RefreshCw, User, Lightbulb, Briefcase
} from 'lucide-react';
...
export default function ExamPage() {
  ...
  const fetchAndShuffleQuestions = async () => {
    const { data, error } = await supabase
      ...
      .eq('course_code', 'ENT 101'); // ENT 101
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.share) {
            ...
            try { await navigator.share({ title: 'Pulsar CBT Result', text: `I scored ${score}/${questions.length} in ENT 101!`, files: [file] }); } catch (err) { console.log(err); }
        } else {
      ...
      <div className="min-h-screen bg-[#09090b] text-white p-6 flex items-center justify-center">
        <div className="w-full max-w-md">
            <div ref={resultCardRef} className="bg-[#111113] border border-zinc-800 p-8 rounded-3xl text-center shadow-2xl mb-6 relative overflow-hidden">
                ...
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-600 mb-8">ENT 101 Score</p>
            ...
            {currentQ.display_options.map((opt: any, idx: number) => {
              ...
              if (isReviewing) {
                 if (isCorrectOption) {
                  ...
                  if (isSelected) btnClass = "bg-orange-600 border-orange-600 text-white shadow-lg shadow-orange-900/20"; 
              }
