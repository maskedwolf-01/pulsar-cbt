...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    if (!currentId) {
      const { data } = await supabase
        .from('chat_sessions')
        .insert({ user_id: user!.id, title: input.slice(0, 20) })
        .select()
        .single();
  ...
  return (
    <div className="flex h-[100dvh] bg-[#050508] text-white font-sans overflow-hidden">
      ...
      {/* SIDEBAR */}
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#0a0a0f] border-r border-white/5 transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0 animate-slide-in-right' : '-translate-x-full'} md:relative md:translate-x-0`}>
        <div className="p-4 flex flex-col h-full">
          <div className="flex items-center gap-2 mb-6">
            <Link href="/dashboard" className="p-2 hover:bg-white/5 rounded-full">
              <ArrowLeft className="w-5 h-5"/>
            </Link>
            <button onClick={createSession} className="flex-1 flex items-center gap-2 bg-purple-600 hover:bg-purple-500 p-2 rounded-xl text-sm font-bold justify-center transition-colors">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-1">
            ...
            {sessions.map(s => (
              <div 
                ...
                className={`group w-full text-left p-3 rounded-lg flex justify-between items-center cursor-pointer ${sessionId === s.id ? 'bg-white/10 text-white' : 'text-gray-400 hover:bg-white/5'} sidebar-session`}
              >
                {editingId === s.id ? (
                  <input 
                    autoFocus 
                    className="bg-black border border-white/20 rounded px-1 w-full text-xs" 
                    value={editTitle} 
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={(e) => { e.stopPropagation(); setEditingId(s.id); setEditTitle(s.title); }} className="p-1 hover:text-white">
      ...
      <div className="flex-1 flex flex-col relative h-full">
        <header className="md:hidden flex items-center justify-between p-4 bg-[#0a0a0f] border-b border-white/5 z-30 flex-none">
          <div className="flex items-center gap-4">
            ...
            <span className="font-bold text-gradient">Nexus 1.0</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center opacity-50 animate-fade-in">
              ...
              <h1 className="text-2xl font-bold text-gradient">Hello, Scholar</h1>
            ...
            messages.map((msg, i) => (
              <div key={i} className={`flex gap-4 mb-6 max-w-3xl mx-auto ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                ...
                <div className={msg.role === 'ai' ? 'chat-message-ai' : 'chat-message-user'}>
                  {msg.text}
                </div>
              </div>
            ))
        ...
        <div className="p-4 bg-gradient-to-t from-black via-black to-transparent z-20 flex-none">
          <div className="max-w-3xl mx-auto bg-[#1a1a1a] border border-white/10 rounded-2xl p-2 flex items-center shadow-2xl">
            <input
              ...
              className="flex-1 bg-transparent border-none text-white px-4 py-2 focus:outline-none blinking-cursor"
              onKeyDown={(e) => e.key === 'Enter' && handleSend()}
            />
            <button
              onClick={handleSend}
              disabled={!input.trim()}
              className={`p-3 rounded-xl ${input.trim() ? 'bg-white text-black' : 'bg-white/5 text-gray-600'}`}
            >
