...
import { Send, Bot, Plus, MessageSquare, Menu, Loader2, Sparkles, Trash2, Edit2, ArrowLeft } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-[#050508] text-white font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col relative h-full">
        <header className="md:hidden flex items-center justify-between p-4 bg-[#0a0a0f] border-b border-white/5 z-30">
        ...
        <div className="flex-1 overflow-y-auto p-4 pb-32 custom-scrollbar">
            {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center opacity-50"><div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center mb-4"><Sparkles className="w-8 h-8 text-purple-500"/></div><h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600">Hello, Scholar</h1><p className="text-subtext mt-2">Start a new session to begin.</p></div>
            ) : ( messages.map((msg, i) => ( <div key={i} className={`flex gap-4 mb-6 max-w-3xl mx-auto ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>{msg.role === 'ai' && <div className="w-8 h-8 rounded-full bg-purple-500/20 flex items-center justify-center mt-1"><Bot className="w-4 h-4 text-purple-400"/></div>}<div className={`p-4 rounded-2xl text-sm leading-relaxed max-w-[85%] ${msg.role === 'ai' ? 'bg-[#1a1a1a] text-gray-200 rounded-tl-none border border-white/5' : 'bg-purple-600 text-white rounded-tr-none'}`}>{msg.text}</div></div> )) )}
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black via-black to-transparent z-20"><div className="max-w-3xl mx-auto bg-[#1a1a1a] border border-white/10 rounded-2xl p-2 flex items-center shadow-2xl"><input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask Nexus..." className="flex-1 bg-transparent border-none text-white px-4 py-2 focus:outline-none" onKeyDown={(e) => e.key === 'Enter' && handleSend()}/><button onClick={handleSend} disabled={!input.trim()} className={`p-3 rounded-xl ${input.trim() ? 'bg-white text-black' : 'bg-white/5 text-gray-600'}`}><Send className="w-5 h-5" /></button></div></div>
      </div>
    </div>
  );
  }
